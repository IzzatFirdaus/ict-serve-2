            ...
            >
                ...
                @switch($theme)
                    @case('light')
                        <x-icon
                            name="bs.sun"
                            class="h-4 w-4 {{ $currentTheme === $theme ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400' }}"
                            aria-hidden="true"
                        />
...
<div class="ds-theme-switcher" role="group" aria-label="{{ __('messages.aria.theme_dropdown') }}">
    @if ($asButton)
        ...
        >
            <li role="option">
                <button type="button" wire:click="switchTheme('system')">
                    {{ __('messages.theme.system') }}
                </button>
            </li>
            <li role="option">
                <button type="button" wire:click="switchTheme('light')">
                    {{ __('messages.theme.light') }}
                </button>
            </li>
            <li role="option">
                <button type="button" wire:click="switchTheme('dark')">
    ...
    @else
        ...
        <select
            ...
            wire:model="theme"
            class="ds-select"
            aria-label="{{ __('messages.aria.theme_select') }}"
        >
