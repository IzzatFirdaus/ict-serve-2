...
<div class="ds-theme-switcher" role="group" aria-label="{{ __('messages.aria.theme_dropdown') }}">
    @if($asButton)
        <button type="button" class="ds-button" aria-haspopup="listbox" aria-expanded="false" id="theme-toggle">
            {{ __('messages.theme.' . ($theme ?? 'system')) }}
        </button>
        <ul role="listbox" aria-labelledby="theme-toggle" class="ds-popover" style="display:none;" id="theme-options">
    ...
    @else
        ...
        <select id="theme-select" wire:change="switchTheme($event.target.value)" wire:model="theme" class="ds-select" aria-label="{{ __('messages.aria.theme_select') }}">
