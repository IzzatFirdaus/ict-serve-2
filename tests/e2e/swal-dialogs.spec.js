...
test_1.test.describe('MYDS SweetAlert2 Dialogs', () => {
    (0, test_1.test)('should show info dialog and be accessible', async ({ page }) => {
        await page.goto('/');
        await page.evaluate(() => {
            window.showSwal && window.showSwal({
                title: 'Maklumat',
        ...
        await (0, test_1.expect)(page.locator('.swal2-html-container')).toHaveText('Ini adalah dialog maklumat.');
    ...
    (0, test_1.test)('should show destructive confirm dialog and handle confirm/cancel', async ({ page }) => {
        await page.goto('/');
        await page.evaluate(() => {
            window.showSwal && window.showSwal({
